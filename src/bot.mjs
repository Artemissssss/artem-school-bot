...
Якщо вам потрібна допомога з використанням цього бота, ось посилання на сторінку з інструкцією: 👉 https://school-doc.artemissssss.de/docs/intro 👈
...
Удачі у використанні бота! 👍😄`)
    ...
    if(text === "Створити клас" && lastUserMessage[msg.from.id] === "/start" && userAction[msg.from.id] === undefined){
        ...
        Раджу прочитати інтрукцію за допомогою /help!
        `, {replyMarkup});
...
bot.on('callbackQuery', async msg => {
    ...
    if(msg.data ==="одобрити"){
        ...
        let newEvent = (userAction[msg.from.id].data).filter(arr => {console.log(arr);return arr.id === msg.data})
        await bot.sendMessage(msg.from.id, `${newEvent[0].text}\n\n\nО ${newEvent[0].date} ${newEvent[0].time}\n\nДля: ${newEvent[0].who}`);
